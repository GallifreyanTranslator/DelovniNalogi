...
function le32(n: number): number[] { return [n & 0xff, (n >> 8) & 0xff, (n >> 16) & 0xff, (n >> 24) & 0xff]; }
...
function buildZip(files: { name: string; content: string }[]): Uint8Array {
  ...
  for (const file of files) {
    ...
    const localHeader = new Uint8Array([
      ...
      ...le16(nameBytes.length), 0x00, 0x00, ...Array.from(nameBytes),
    ]);
  ...
  for (const entry of entries) {
    ...
    centralParts.push(new Uint8Array([
      ...
      ...le32(entry.crc), ...le32(entry.data.length), ...le32(entry.data.length),
      ...le16(nameBytes.length), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      ...le32(entry.offset), ...Array.from(nameBytes),
    ]));
  }
  ...
  const eocd = new Uint8Array([
    ...
    ...le32(centralDir.length), ...le32(localData.length), 0x00, 0x00,
  ]);
...
function buildSheetXml(rows: (string | number)[][]): string {
  ...
  rows.forEach((row, ri) => {
    ...
    row.forEach((cell, ci) => {
      ...
      else xml += `<c r="${ref}" t="inlineStr"><is><t>${escXml(cell)}</t></is></c>`;
    });
    xml += `</row>`;
  });
...
function buildWorkersData(records: WorkRecord[]): (string | number)[][] {
  ...
  records.forEach(r => {
    ...
    if (!map.has(key)) map.set(key, { row: [r.obcina, r.operacija, r.oseba, r.osebaValue, r.tipOsebe === 'voznik' ? 'Voznik' : 'Sodelavec', 0, r.datum], ure: 0 });
    map.get(key)!.ure += r.ure;
  });
  const rows: (string | number)[][] = [header];
  map.forEach(({ row, ure }) => { row[5] = Math.round(ure * 10) / 10; rows.push(row); });
  return rows;
}
...
function buildVehiclesData(records: WorkRecord[]): (string | number)[][] {
  ...
  records.filter(r => r.tipOsebe === 'voznik').forEach(r => {
    ...
    if (!map.has(key)) map.set(key, { row: [r.obcina, r.operacija, r.vozilo, r.voziloValue, 0, r.datum], ure: 0 });
    map.get(key)!.ure += r.ure;
  });
  const rows: (string | number)[][] = [header];
  map.forEach(({ row, ure }) => { row[4] = Math.round(ure * 10) / 10; rows.push(row); });
  return rows;
}

function buildXlsxFiles(sheets: { xml: string; name: string }[]): { name: string; content: string }[] {
  const sheetFiles = sheets.map((s, i) => ({ name: `xl/worksheets/sheet${i + 1}.xml`, content: s.xml }));
  ...
  const sheetDefs = sheets.map((s, i) => `<sheet name="${escXml(s.name)}" sheetId="${i + 1}" r:id="rId${i + 1}"/>`).join('');
  return [
    ...
    { name: 'xl/workbook.xml', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${sheetDefs}</sheets></workbook>` },
    { name: 'xl/_rels/workbook.xml.rels', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${sheetRels}</Relationships>` },
    ...sheetFiles,
  ];
}

async function shareXlsx(rows: (string | number)[][], sheetName: string, filename: string): Promise<void> {
  const sheetXml = buildSheetXml(rows);
  const zip = buildZip(buildXlsxFiles([{ xml: sheetXml, name: sheetName }]));
  const b64 = u8ToBase64(zip);
  const fileUri = (FileSystem.documentDirectory ?? '') + filename;
  await FileSystem.writeAsStringAsync(fileUri, b64, { encoding: FileSystem.EncodingType.Base64 });
  const avail = await Sharing.isAvailableAsync();
  if (avail) await Sharing.shareAsync(fileUri, { mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', dialogTitle: sheetName, UTI: 'org.openxmlformats.spreadsheetml.sheet' });
}
